...
### `cat` Command – Quick Guide

- **`cat filename.txt`** – Displays the entire file content in the terminal.

**Other Useful Commands**:

- **`less filename.txt`** – Opens the file for scrolling.
- **`more filename.txt`** – Similar to `less`, but for forward navigation only.
